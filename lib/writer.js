...
export function write(ast, { templates: ts = templates } = {}) {
  ...
  function* trivia(triv) {
    if (triv) {
      yield ts.trivia(triv);
    }
  }
  ...
  function token(t, wrapper = noop, ...args) {
    ...
    return ts.wrap([...trivia(t.trivia), value]);
  }
  ...
  function type_body(it) {
    ...
    return ts.wrap([...trivia(firstToken.trivia), ref]);
  }
  function type(it) {
    return ts.wrap([
